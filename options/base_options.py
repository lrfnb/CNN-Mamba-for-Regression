...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--seed', type=int, default=11, help='Random seed for training')
        parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints', help='models are saved here')
        parser.add_argument('--debug', action='store_true', help='if specified, set to debug mode')
        # model parameters
        parser.add_argument('--model', type=str, default='drn', help='chooses which model to train [parse|enhance]')
        parser.add_argument('--att_name', type=str, default='drn', help='attention type [drn|spar3d]')
    ...
    def parse(self):
        ...
        if opt.gpus > 0:
            ...
            opt.data_device = torch.device('cuda:{}'.format(opt.gpu_ids[2 % opt.gpus]))
        else:
            opt.gpu_ids = []
